...
from ryebot.bot.cli.wiki_manager import (display_wiki_list, add_wiki, remove_wiki,
    go_online_on_wiki, go_offline_on_wiki)
...
def main():
    """Entry function called from the command line  via `$ ryebot`.

    This is defined in `setup.py`
    """
    # the code in this function is always executed when the command "ryebot"
    # is called from the command line, no matter the arguments and options,
    # that is unless one of the options that exit the app in their callback
    # (help/version/debug, i.e. --debug, -h, -v, etc.) is used (because the
    # parser doesn't reach this function in that case)
    start_daemon()
...
@click.option('-w', '--wiki', multiple=True,
    help='Display status only for this wiki (can be used multiple times).')
...
def main_list():
    ...
    click.echo('\n'.join((
        ...
        'Use "ryebot scriptinfo" to get information about a script, '
        'or "ryebot start" or "ryebot stop" to start/stop one.'
    )))


@click.command(context_settings=CONTEXT_SETTINGS, name='scriptinfo')
@click.option('-s', '--scriptname', required=True,
    help='Name of the script for which to display info')
def main_scriptinfo(scriptname):
    """Display information about a script."""
    if scriptname not in __availablescripts__:
        click.echo(f'The script "{scriptname}" is not available. '
            'See "ryebot list" for a list of available scripts.')
    else:
...
@click.option('-w', '--wiki', multiple=True,
    help='Wiki on which to go online (can be used multiple times).')
...
@click.option('-w', '--wiki', multiple=True,
    help='Wiki on which to go offline (can be used multiple times).')
...
@click.option('-s', '--scriptname',
    type=click.Choice(__availablescripts__, case_sensitive=False),
    prompt='Name of the script to start')
...
@click.option('-c', '--count', is_flag=True,
    help='Display only the number of wikis.')
def wiki_list(count):
...
@click.option('-n', '--name', prompt='Name of the wiki',
    help='Name of the wiki to add. Use "<wikiname>/<lang>" for language variants.')
def wiki_add(name):
...
@click.option('-n', '--name', prompt='Name of the wiki',
    help='Name of the wiki to remove. Use "<wikiname>/<lang>" for language variants.')
@click.confirmation_option('-y', '--yes',
    prompt='Are you sure you want to withdraw access from this wiki? '
    'This will permanently delete all local information that the bot has '
    'collected about the wiki, such as logs about script executions on the wiki.')
